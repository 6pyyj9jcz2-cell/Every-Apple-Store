...
## View vs. edit access
By default, a trip code is **view-only** — everyone who has it can see all
11 stops, checkmarks, ratings, and notes, but the checkboxes, rating boxes,
and note fields are disabled, so nothing they touch actually saves.

To get edit access, add `-e` to the end of the code (e.g. `jamie-nyc-2026-e`).
That's the version you'd keep for yourself and anyone else who should
actually be able to check off stores, rate them, and leave notes.

Like the trip code itself, this is a convenience, not real security: since
the edit code is just the plain code with `-e` appended, anyone who has it
could type the base code instead to view it, or guess the `-e` suffix to
get edit access. Fine for keeping accidental taps from messing up your
progress; don't rely on it to keep someone out who's determined to edit it.
