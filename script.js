...
function pushToCloud(state) {
  if (!db || !getTripCode() || !isEditorCode()) return;
  clearTimeout(pushTimer);
  setSyncStatus("Saving…");
  pushTimer = setTimeout(() => {
    fs.setDoc(tripDocRef(getBaseCode()), { state, updatedAt: Date.now() })
...
function isEditorCode() {
  const raw = getTripCode();
  return !!raw && raw.endsWith("-e");
}

function getBaseCode() {
  const raw = getTripCode();
  if (!raw) return null;
  return isEditorCode() ? raw.slice(0, -2) : raw;
}
...
async function unlockWithCode(rawCode) {
  ...
  if (db) {
    await syncFromCloud(getBaseCode());
    listenForRemoteChanges(getBaseCode());
  } else {
...
function render() {
  ...
  STORES.forEach((store, i) => {
    ...
    li.innerHTML = `
      ...
      <div class="spine ${store.leg ? "mode-" + legMode : ""} ${entry.visited ? "is-done" : ""}"></div>
      ${store.leg ? `
        <div class="leg">
          <span class="leg-chip mode-${legMode}">${store.leg.chip}</span>
          <span>${legTypeLabel(store.leg.mode)} · ${store.leg.duration} from stop ${i}</span>
        </div>
      ...
      <div class="card">
        <div class="card-top">
          ...
          <label class="visited-toggle">
            <input type="checkbox" class="visited-input" ${entry.visited ? "checked" : ""} ${dis} />
            Visited
          </label>
        </div>
        ...
        <div class="card-controls">
          <div class="rating-block">
            ...
            <input type="number" min="0" max="10" step="0.5" class="rating-input" placeholder="—" value="${entry.rating}" ${dis} />
          </div>
          <div class="notes-block">
            <span class="notes-label">Notes</span>
            <textarea class="notes-input" placeholder="How was it?" ${dis}>${entry.notes}</textarea>
          </div>
        </div>
      </div>
    `;
  ...
  badge.textContent = viewerMode ? "👀 View only — changes here won't be saved" : "";
  document.getElementById("reset-btn").disabled = viewerMode;
...
function attachHandlers() {
  document.querySelectorAll(".stop").forEach(li => {
    ...
    visitedInput.addEventListener("change", () => {
      ...
      li.querySelector(".spine").classList.toggle("is-done", visitedInput.checked);
      updateProgress(s);
      if (visitedInput.checked) showPhotoReminder();
    });

    ratingInput.addEventListener("input", () => {
      let num = parseFloat(ratingInput.value);
      if (!isNaN(num)) {
        if (num > 10) { ratingInput.value = "10"; }
        if (num < 0) { ratingInput.value = "0"; }
      }
      const s = loadState();
